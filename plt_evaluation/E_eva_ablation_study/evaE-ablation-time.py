...
# 加载数据
df = pd.read_csv('ablation_data_time.csv')
...
stage1_only = df['only Stage1'].fillna(0).values.tolist()  # 处理NaN为0
stage2_only = df['only Stage2'].fillna(0).values.tolist()  # 处理NaN为0
stage1_2 = df['Stage1 + Stage2'].fillna(0).values.tolist()  # 处理NaN为0
